...
static void StartPressed()
{
	...
	if (pMesh->GetAllConnections().size() < numHumanPlayers - 1)
	{
		...
		GadgetListBoxAddEntryText(listboxGameSetupChat, text, GameSpyColor[GSCOLOR_MOTD_HEADING], -1, -1);
		...
		for (LobbyMemberEntry& lobbyMember : vecLobbyMembers)
		{
			if (lobbyMember.IsHuman() && lobbyMember.user_id != myUserID) // dont show AI or self
			{
				...
				if (!bFoundLobbyMemberForConnection)
				{
					...
					GadgetListBoxAddEntryText(listboxGameSetupChat, strDisplayName, GameSpyColor[GSCOLOR_MOTD_HEADING], -1, -1);
				}
			}
		}
	...
	if(isReady)
	{
		...
		GadgetListBoxAddEntryText(listboxGameSetupChat, strInform, GameSpyColor[GSCOLOR_MOTD_HEADING], -1, -1);
			...
			pWS->SendData_StartFullMeshConnectivityCheck([=](bool bMeshFullyConnected, std::list<std::pair<int64_t, int64_t>> missingConnections)
				{
					if (bMeshFullyConnected)
					{
						...
						GadgetListBoxAddEntryText(listboxGameSetupChat, strInform, GameSpyColor[GSCOLOR_ACCEPT_TRUE], -1, -1);
					...
					else
					{
						...
						GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Connections: Missing links:"), GameSpyColor[GSCOLOR_MOTD_HEADING], -1, -1);
						for (auto& missingPair : missingConnections)
						{
							...
							if (pLobbyInterface != nullptr)
							{
								...
								if (lobbyMemberSource.user_id != -1 && lobbyMemberTarget.user_id != -1)
								{
									...
									GadgetListBoxAddEntryText(listboxGameSetupChat, strMissingConnection, GameSpyColor[GSCOLOR_MOTD_HEADING], -1, -1);
								}
							}
				...
				else
				{
					...
					if (connState == EConnectionState::CONNECTION_FAILED)
					{
						...
						const Color connectionColor = connState == EConnectionState::CONNECTION_FAILED
							? GameSpyColor[GSCOLOR_ACCEPT_FALSE]
...
// NGMP: Did we just enter a lobby with modified camera height?
    ...
    if (pLobbyInterface != nullptr)
    {
        if (pLobbyInterface->IsHost())
        {
			GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby access: Anyone can join. Use /friendsonly to limit the lobby to friends."), GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
        }
    }
    ...
    if (TheNGMPGame != nullptr)
	{
		...
		{
			GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Observers: Disabled by the host."), GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
		}
	}
#endif
				...
				if (pLobbyInterface->m_bHostMigrated)
				{
					...
					if (!TheNGMPGame->isGameInProgress()) // in progress is in game, ingame is just in lobby
					{
						...
						if (bIsHost)
						{
							...
							GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Host: The previous host left. You are now the host."), GameSpyColor[GSCOLOR_DEFAULT], -1, -1);

							// NOTE: don't need to mark ourselves ready, the service did it for us upon migration
						}
						else
						{
							GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Host: The previous host left. A new host was selected."), GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
						}
...
Bool handleGameSetupSlashCommands(UnicodeString uText)
{
		...
		if (pOnlineServicesMgr != nullptr)
		{
			...
			if (pLobbyInterface != nullptr)
			{
				if (pLobbyInterface->IsInLobby())
				{
					...
					{
						GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby access: Friends only. Use /public to let anyone join."), GameSpyColor[GSCOLOR_ACCEPT_TRUE], -1, -1);
        ...
        if (pOnlineServicesMgr != nullptr)
        {
            ...
            if (pLobbyInterface != nullptr)
            {
                if (pLobbyInterface->IsInLobby())
                {
					if (pLobbyInterface->IsHost()) // NOTE: this is checked service side too, but we might as well not make the call to reduce resource usage
                    {
						GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby access: Anyone can join. Use /friendsonly to limit the lobby to friends."), GameSpyColor[GSCOLOR_ACCEPT_TRUE], -1, -1);
		...
		if (pOnlineServicesMgr != nullptr)
		{
			...
			if (pLobbyInterface != nullptr)
			{
				if (pLobbyInterface->IsInLobby())
				{
					if (pLobbyInterface->IsHost())
					{
						...
						if (bIsNumber)
						{
							...
							if (newCameraHeight < GENERALS_ONLINE_MIN_LOBBY_CAMERA_ZOOM || newCameraHeight > GENERALS_ONLINE_MAX_LOBBY_CAMERA_ZOOM)
							{
								...
								GadgetListBoxAddEntryText(listboxGameSetupChat, msg, GameSpyColor[GSCOLOR_ACCEPT_FALSE], -1, -1);
								return TRUE; // was a slash command
							}
							else
							{
						...
						{
							GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Camera height: Enter a number."), GameSpyColor[GSCOLOR_ACCEPT_FALSE], -1, -1);
							return TRUE; // was a slash command
						}
					...
					{
						GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Camera height: Only the host can change it."), GameSpyColor[GSCOLOR_ACCEPT_FALSE], -1, -1);
						return TRUE; // was a slash command
					}
				}
			}
	...
	else if (token == "steam" || token == "advnet")
	{
		...
		GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Network debug: Statistics were written to the log file."), GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
		...
		if (pLobbyInterface != nullptr && pAuthInterface != nullptr)
		{
			...
			if (theLobby.owner == pAuthInterface->GetUserID())
			{
				if (newPassword.getLength() == 0 || newPassword.getLength() > GENERALS_ONLINE_LOBBY_MAX_PASSWORD_LENGTH)
				{
					...
					GadgetListBoxAddEntryText(listboxGameSetupChat, errorMsg, GameSpyColor[GSCOLOR_ACCEPT_FALSE], -1, -1);
				}
				else
				{
					std::shared_ptr<WebSocket>  pWS = NGMP_OnlineServicesManager::GetWebSocket();
					if (pWS != nullptr)
					{
						...
						GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby password: Updated. Use /removepassword to remove it."), GameSpyColor[GSCOLOR_ACCEPT_TRUE], -1, -1);
					}
				}
			}
			else
			{
				GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby password: Only the host can change it."), GameSpyColor[GSCOLOR_ACCEPT_FALSE], -1, -1);
			}
		}
			...
			if (theLobby.owner == pAuthInterface->GetUserID())
			{
				if (theLobby.passworded)
				{
					...
					if (pWS != nullptr)
					{
						...
						GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby password: Removed. Use /setpassword <password> to add one."), GameSpyColor[GSCOLOR_ACCEPT_TRUE], -1, -1);
					}
				}
				else
				{
					GadgetListBoxAddEntryText(listboxGameSetupChat, UnicodeString(L"Lobby password: No password is set. Use /setpassword <password> to add one."), GameSpyColor[GSCOLOR_DEFAULT], -1, -1);
				}
			}
			else
			{
